...
class mpc_config:
    ...
    map_root : str = "/sim_ws/src/f1tenth_gym_ros/maps/"
    ...
    Rk: list = field(
        default_factory=lambda: np.diag([10.1, 100.1])
    ...
    Qk: list = field(
        ...
        default_factory=lambda: np.diag([70., 70., 250.5, 250.0])
    ...
    MAX_DSTEER: float = np.deg2rad(180.0)  # maximum steering speed [rad/s]    
    MAX_SPEED: float = 9.0  # maximum speed [m/s]
    MIN_SPEED: float = 0.0  # minimum backward speed [m/s]
    MAX_ACCEL: float = 1.0  # maximum acceleration [m/ss]
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        # New way to load the maps
        map_root = mpc_config.map_root + self.map_name + '/' + self.map_name + '_centerline.csv'
        #folder inside the container: /sim_ws/src/f1tenth_gym_ros/maps
        self.waypoints = np.loadtxt(map_root, delimiter=',', skiprows=1)
        ...
        self.ref_speed = np.ones(len(self.waypoints)) * 7.5  # speed profile
    ...
    #     # 1) compute u_ref from ref_traj
    #     TK, DTK, WB = self.config.TK, self.config.DTK, self.config.WB
    #     a_ref     = np.zeros(TK)
    #     delta_ref = np.zeros(TK)
    #     for t in range(TK):
    #         v0, v1 = ref_traj[2,t], ref_traj[2,t+1]
    #         a_ref[t] = (v1 - v0)/DTK
    #         yaw0, yaw1 = ref_traj[3,t], ref_traj[3,t+1]
    #         yaw_rate   = (yaw1 - yaw0)/DTK
    #         delta_ref[t] = math.atan((yaw_rate*WB)/max(v0,1e-3))

    #     # 2) build big A, B, C blocks around (x_ref, u_ref)
    #     A_block, B_block, C_block = [], [], []
    #     for t in range(TK):
    #         x_op      = ref_traj[:, t]       # [x,y,v,yaw]
    #         a_op      = a_ref[t]
    #         delta_op  = delta_ref[t]

    #         # Jacobians
    #         A_t, B_t, _ = self.get_model_matrix(
    #             v     = x_op[2],
    #             phi   = x_op[3],
    #             delta = delta_op
    #         )

    #         # exact one‐step update
    #         x_next = np.array([
    #             x_op[0] + x_op[2]*math.cos(x_op[3])*DTK,
    #             x_op[1] + x_op[2]*math.sin(x_op[3])*DTK,
    #             x_op[2] + a_op*DTK,
    #             x_op[3] + (x_op[2]/WB)*math.tan(delta_op)*DTK
    #         ])

    #         # offset C_t = f(x_op, u_op) – A_t x_op – B_t u_op
    #         u_op = np.array([a_op, delta_op])
    #         C_t  = x_next - A_t.dot(x_op) - B_t.dot(u_op)
    ...
    #     # 3) update your cvxpy Params
    #     self.Annz_k.value = A_block.data
    #     self.Bnnz_k.value = B_block.data
    #     self.Ck_.value    = C_block

    #     # 4) solve as before
    #     self.MPC_prob.solve(solver=cvxpy.OSQP, warm_start=False)
